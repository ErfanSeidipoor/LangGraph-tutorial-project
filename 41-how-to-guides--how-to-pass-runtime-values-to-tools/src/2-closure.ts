...
(async () => {
  ...
  const toolNodeWithClosure = async (
  ...
  const workflow = new StateGraph(MessagesAnnotation)
    ...
    .addNode("tools", toolNodeWithClosure)
    .addEdge(START, "agent")
  ...
  await fs.writeFile("./diagram-closure.png", Buffer.from(await buffer));
  ...
  // prettier-ignore
  fs.writeFile("RESULT-closure.json",JSON.stringify(RESULT, null, 2),"utf8");
})();

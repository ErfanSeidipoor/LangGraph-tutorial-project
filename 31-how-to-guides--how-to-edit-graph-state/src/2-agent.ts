...
(async () => {
  ...
  const workflow = new StateGraph(AgentState)
    ...
    .addConditionalEdges("agent", shouldContinue, ["action", END])
  ...
  const updatedStateToolCalls =
    newState.values.messages[newState.values.messages.length - 1].tool_calls;
  console.log(updatedStateToolCalls);
  RESULT.push({ updatedStateToolCalls });
